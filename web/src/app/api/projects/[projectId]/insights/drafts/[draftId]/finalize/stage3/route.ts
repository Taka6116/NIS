...
import { runInsightStage3, type InsightProvider } from "@/lib/insights/run-generate";
...
function parseIssues(body: unknown): InsightIssue[] | undefined {
  ...
  for (const item of raw) {
    ...
    out.push({ id: r.id, severity, title: r.title, description: r.description, relatedFactIds: related, category });
  }
  return out;
}
...
export async function POST(
  ...
  try {
    const result = await runInsightStage3(projectId, draftId, provider, { editedIssues });
    ...
    console.error("[finalize/stage3] error:", message, stack);
    return Response.json(
